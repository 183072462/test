...
class FeapointsController extends CommonController{
   ...
   function _filter(&$map) {
	  ...
	  $this->classifylist=D('Datalist')->getselect(36);
  ...
  public function _befor_update($data){
   ...
   public function _befor_view($id){
	   $wheid['id']=$id;
	   $plist=D('feapoints')->where($wheid)->field('classify,attid')->find();
	   //获取荣誉级别levelid
   ...
   public function outxls() {
		...
		$list = $model->where($map)->field('id,title,uname,addtime,classify,points')->select();
		
		foreach($list as $K => &$v){
			$v['classify']=D('Datalist')->getselectname($v['classify']);
			}
	    $headArr=array('ID','标题','添加单位','添加时间','分类','分数');
	    $filename='我的文档';
